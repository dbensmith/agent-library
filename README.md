...
- `.apm/instructions/`: System instructions for agent behavior.
- `.apm/skills/`: Tool definitions and specialized agent capabilities (each in its own folder).
- `.apm/agents/`: Agent definitions with frontmatter metadata.
- `.agents/`: Deployed runtime skills and agents (managed by APM).
- `build/`: Compiled package output (generated by `apm pack`).
- `docs/`: APM readiness review artifacts and remediation plans.
